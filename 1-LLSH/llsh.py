...
class LLSH:
    '''
    Implement 'Index' and 'Query' stages.
    '''
    ...
    def cvt_key(self, hash_code: np.ndarray) -> str:
        """
        convert a hash_code (e.g. [0 1 0 1]) to string ('0101').
        """
    ...
    def batch_index(self, vecs: torch.Tensor) -> None:
        """
        Index a batch of 1-D vectors.
        vecs: (N, ddim)
        """
        ...
        for _b, (_table_b, _bin_code_b, _real_code_b) in enumerate(zip(self.hash_table, _bin_codes, _hash_codes)):  # b * [N, r]
            for _bin_code_b_n, _real_code_b_n in zip(_bin_code_b, _real_code_b):  # N * [r,]
                _table_b[self.cvt_key(_bin_code_b_n)].append(_real_code_b_n)  # [r,]

        if self.__verbose:
            print(f"{ttime()-t1:.3f}s")

    def cvt_list2tensor(self) -> None:
        '''
        For each bucket, convert the list of tensors to a stack of tensors.
        It should be used after finishing `batch_index()`.
        '''
        for _b, _table_b in enumerate(self.hash_table):
            for _k in _table_b.keys():
                if not _table_b[_k] == []:
                    _table_b[_k] = torch.stack(_table_b[_k])
                else:
                    _table_b[_k].remove([])

    def batch_query(self, vecs: torch.Tensor) -> torch.Tensor:
        """
        Query a batch of 1-D vectors.
        vecs: (M, ddim)
        ...
        for _bin_code_m, _real_code_m in zip(_bin_codes, _hash_codes):  # M * [b, r]
            ...
            for _b, (_table_b, _bin_code_m_b, _real_code_m_b) in enumerate(zip(self.hash_table, _bin_code_m, _real_code_m)):  # b * [r,]
                _cand_list = _table_b[self.cvt_key(_bin_code_m_b)]
                if not _cand_list == []:
                    _dists: torch.Tensor = torch.norm(_cand_list - _real_code_m_b, p=2, dim=-1)
                    assert len(_dists.shape) == 1 and _dists.shape[0] == len(_cand_list), _dists.shape

                    _candist_b.append(_dists.mean())
                else:
                    _candist_b.append(torch.as_tensor(1e10, dtype=torch.float32))
