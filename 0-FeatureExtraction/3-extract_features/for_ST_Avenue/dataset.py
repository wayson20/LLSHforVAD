...
class VAD_Dataset(tc_Dataset):
    def __init__(self, root_dir: str, snippet_len: int, snippet_itv: int, frm_name_len: int):
        '''
    ...
    def _sample_all_frms(self, vid_name) -> torch.Tensor:
        frm_path_list = [join(self.root_dir, vid_name, f"{str(frm_idx).zfill(self.frm_name_len)}.jpg") for frm_idx in range(self.vid_len_dict[vid_name])]
        return torch.stack([read_image(frm_path) for frm_path in frm_path_list])
