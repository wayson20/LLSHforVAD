...
def cal_macro_auc(score_dict: dict, gt_npz: np.lib.npyio.NpzFile):
    '''
    Calculate macro-AUC.
    '''
    auc_dict = {}
    ...
    for vid_name in vid_name_list:
        vid_gts: np.ndarray = gt_npz[vid_name]
...
def cal_micro_auc(score_dict: dict, gt_npz: np.lib.npyio.NpzFile):
    '''
    Calculate micro-AUC.
    '''
    cat_gts = []
